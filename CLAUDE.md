...
- **Clean Formula mode** — tries to recognize the drawing as one elegant equation from a curated library (polynomials, sine, exponential, power, rational, gaussian, logarithm, tangent, a polynomial+sine composite, circle). Falls back to a best-effort answer with a warning when nothing fits well or the shape isn't one of the recognized families.
...
Both paths render via `canvas/CurveRenderer.ts` and display via `display/EquationDisplay.tsx` (KaTeX). `EquationDisplay` (Clean Formula's single equation) and `PiecewiseEquationDisplay` (Spline mode's per-segment rows) each render a `display/CopyButton.tsx` that copies the raw LaTeX string via `navigator.clipboard` — `PiecewiseEquationDisplay` gets both one whole-block button (joins every row) and one per-row button, since a user may want just one segment. This works only because this codebase's LaTeX is already backslash-free plain text (see the KaTeX gotcha below) — there's no separate plain-text rendering to build.
...
Both entry points smooth via `preprocessing/smooth.ts`'s `movingAverage(points, windowSize, iterations)`. `iterations` (default 1) reruns the same single-pass box filter, feeding each pass's output into the next — iterating a small window approximates a Gaussian kernel and rejects hand-tremor jitter far better than one single wider pass. `pipeline.ts` calls it with `SMOOTH_ITERATIONS = 3` at both call sites. Note this only reduces per-point jitter magnitude — it does not (and structurally cannot) make a spline through 200 densely-packed knots perfectly straight, since `naturalCubicSpline` interpolates every knot exactly and curvature at a knot scales roughly like `noise/spacing^2`. Tightening `SMOOTH_ITERATIONS`/`SMOOTH_WINDOW` further trades off against lag/corner-cutting at stroke endpoints; fixing the wiggle at its root would require changing knot placement (e.g. adaptive simplification) rather than just smoothing harder, which has been discussed but not implemented as of this writing.
...
2. **y=f(x) function families** (`fitting/formulas/families/*.ts`) — one fitter per family (constant/linear/quadratic/cubic/quartic via `ml-regression-polynomial`; sine/gaussian/tangent via `ml-levenberg-marquardt` with multiple initial-guess candidates since nonlinear least squares is sensitive to seeding; exponential via log-linearization + LM refinement; power/rational/logarithm via closed-form log-log, 1/x, or log-x linear regression, domain-gated to x>0 or x-away-from-0 respectively; composite via a low-degree polynomial trend plus a sine fit to that trend's residuals, reusing the polynomial and sine fitters directly rather than a general basis search). Tangent additionally guards against vertical asymptotes: after fitting, it re-samples `predict(x)` densely across the observed domain and rejects the candidate (`null`) if any sample is non-finite or far outside the data's own y-range, since an asymptote landing inside the drawn domain would otherwise blow up into a numerically meaningless "fit." Each fitter returns `null` when its preconditions aren't met rather than fitting garbage. `scoring.ts`: `score = NRMSE + 0.015 * paramCount` — the small param-count penalty only breaks near-ties toward the simpler family, never overrides a genuinely better fit (this is also what keeps the composite family from winning over a single family on a trivial improvement, without any extra threshold logic). `fitCleanFormula.ts` runs every family, scores them, returns the best plus `isPoorFit` (NRMSE > 0.1). A poor fit is still shown with a warning, never blocked — this was an explicit product decision, and it's why a non-circular closed shape (square, spiral, scribble) still gets *some* answer (usually "Constant") rather than nothing.
...
- The sine, tangent, and logarithm families write literal `sin(...)`, `tan(...)`, `ln(...)` (plain letters, no backslash) instead of `\sin(...)`/`\tan(...)`/`\ln(...)`.
...
- `drawFunctionCurve()` (used by Clean Formula mode's y=f(x) fallback) takes an optional `xDomain` option; when omitted it falls back to the full `[-10,10]` viewport. **This is used deliberately in `App.tsx`**: Clean Formula mode intentionally does not pass a clipped `xDomain`, so the fitted function is evaluated (and extrapolates) across the entire viewport regardless of how little of that range was actually drawn in — a cubic can shoot off-screen fast, and a periodic sine/tangent can repeat into phantom extra oscillations never drawn. This was an explicit product decision to make fitted curves "go on forever" rather than stop at the stroke's edges; if you need the old clipped-to-stroke behavior for some new call site, pass `xDomain` explicitly rather than relying on the default. `drawParametricCurve()` (Spline mode, circle fit) doesn't have this consideration since it's bounded to `t∈[0,1]` by construction.
